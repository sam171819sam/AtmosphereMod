package net.mcreator.atmosphere.renderer;
...
public abstract class RotatableEntityRenderer<T extends LightningLurkerEntity & IAnimatable> extends EntityRenderer<T>
	...
	public RotatableEntityRenderer(EntityRendererProvider.Context renderManager, AnimatedGeoModel<T> modelProvider) {
		super(renderManager);
	...
	public void render(T animatable, float entityYaw, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource,
		...
		float lerpXBodyRot = Mth.lerp(partialTick, animatable.xBodyRotO, animatable.xBodyRot);
      	float lerpZBodyRot = Mth.lerp(partialTick, animatable.zBodyRotO, animatable.zBodyRot);
		...
		applyRotations(animatable, poseStack, ageInTicks, lerpBodyRot, lerpXBodyRot, lerpZBodyRot, partialTick);
	...
	/**
	 * Use {@link RotatableEntityRenderer#getOverlay(T, float)}<br>
	 * Remove in 1.20+
	 */
	@Deprecated(forRemoval = true)
	...
	protected void applyRotations(T animatable, PoseStack poseStack, float ageInTicks, float rotationYaw,
			float bodyRotationX, float bodyRotationY, float partialTick) {
		...
		if (pose != Pose.SLEEPING)
    		...
    		poseStack.mulPose(Vector3f.YP.rotationDegrees(bodyRotationY));
    		poseStack.translate(0.0D, (double)-1.2F, 0.0D);
	...
	public <E extends Entity> void renderLeash(T entity, float partialTick, PoseStack poseStack,
		...
		for (int segment = 0; segment <= 24; ++segment) {
			RotatableEntityRenderer.renderLeashPiece(vertexConsumer, posMatrix, xDif, yDif, zDif, entityBlockLight, holderBlockLight,
