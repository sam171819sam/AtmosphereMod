...
public class LuminumOreFeature extends OreFeature {
	...
	public static Feature<?> feature() {
		...
		CONFIGURED_FEATURE = FeatureUtils.register("atmosphere:luminum_ore", FEATURE,
				new OreConfiguration(List.of(OreConfiguration.target(new BlockStateMatchTest(AtmosphereModBlocks.SHATTER_STONE.get().defaultBlockState()), AtmosphereModBlocks.LUMINUM_ORE.get().defaultBlockState())), 8));
		PLACED_FEATURE = PlacementUtils.register("atmosphere:luminum_ore", CONFIGURED_FEATURE,
				List.of(CountPlacement.of(9), InSquarePlacement.spread(), HeightRangePlacement.uniform(VerticalAnchor.absolute(0), VerticalAnchor.absolute(200)), BiomeFilter.biome()));
		return FEATURE;
	}
